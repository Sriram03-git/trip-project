...
function toggleSidebar() {
  ...
  if (window.innerWidth <= 480) {
    ...
    if (sidebar.classList.contains("collapsed")) {
      ...
      pageContainer.style.marginLeft = "250px";
    }
  }
}
...
function renderSettlements(settlements) {
  ...
  if (settlements.length === 0) {
    settlementDiv.innerHTML =
      '<p class="settled-message">Everyone is settled up! Zero transactions needed.</p>';
    return;
  }
  let html = "<ul>";
...
function renderChart(settlements) {
  ...
  let finalChartType =
    chartType === "horizontalBar" || chartType === "bar" ? "bar" : chartType;
      ...
      datasets: [
        {
          ...
          borderColor:
            finalChartType === "line" ? "rgba(26, 115, 232, 0.7)" : undefined,
          borderWidth: 2,
          type: datasetType,
...
function renderSpendingBreakdown(expenses) {
  ...
  if (totalSpent === 0) {
    summaryDiv.innerHTML =
      '<p style="text-align: center; color: var(--text-color);">No expenses added yet.</p>';
  ...
  // Render Text Summary
  let summaryHtml = `<p style="font-weight: 700; color: var(--text-color);">Total Spend: ₹${totalSpent.toFixed(
    2,
  )}</p><ul>`;
  Object.entries(userContribution).forEach(([id, amount]) => {
    const name = usersMap[id] || `ID ${id}`;
    ...
    summaryHtml += `
            <li>
                <span>${name}:</span>
                <span style="font-weight: 700; color: var(--primary-color);">₹${amount.toFixed(
                  2,
                )} (${percentage}%)</span>
            </li>
        `;
...
function renderCategoryBreakdown(categoryData) {
  ...
  if (categoryData.length === 0) {
    categoryDiv.innerHTML =
      '<p style="text-align: center; color: var(--text-color);">No categorized expenses found.</p>';
    return;
  }
  ...
  categoryData.forEach((item) => {
    const topSpenderName =
      usersMap[item.topSpenderId] || `ID ${item.topSpenderId}`;
    ...
    html += `
            <div class="category-card">
                ...
                <p>
                    <span style="font-weight: 700;">Total Spent: ₹${parseFloat(
                      item.totalCategorySpend,
                    ).toFixed(2)}</span>
                    <br>
  ...
  try {
    ...
    if (response.ok) {
      messageElement.textContent =
        "Expense saved successfully! Recalculating debt...";
      messageElement.classList.add("success");
      form.reset();
      await fetchDataAndRender();
    } else {
